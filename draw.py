...
class Draw(object):
    ...
    def distribution(self, x, binning=None, title='', q=.02, lf=.2, rf=.2, r=None, **kwargs):
        if hasattr(x, 'GetName'):
            th = x
        else:
            th = TH1F(Draw.get_name('h'), title, *choose(binning, find_bins, values=x, q=q, lfac=lf, rfac=rf, r=r))
            fill_hist(th, x)
